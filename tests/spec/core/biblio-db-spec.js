...
describe("Core - biblioDB", () => {
  ...
  describe("add() method", () => {
    it("rejects when adding bad types", done => {
      biblioDB
        .add("invalid", "bar")
        .catch(err => expect(err instanceof TypeError).toBe(true))
        .then(done);
    });
  ...
  describe("isAlias() method", () => {
    ...
    it("returns true when it is an alias", done => {
      biblioDB
        ...
        .then(() => biblioDB.isAlias("test-isAlias-pass"))
        .then(result => expect(result).toBe(true))
        .then(done);
    });
  ...
  describe("find() method", () => {
    ...
    it("finds a references and resolves aliases", done => {
      biblioDB
        .addAll(data)
        .then(() => biblioDB.find("DAHU"))
        .then(result => expect(result.id).toEqual("DAHUT"))
        .then(
          () => biblioDB.find("whatwg-dom") // alias
        )
        .then(result => expect(result.title).toEqual("DOM Standard"))
        .then(done);
    });
  });
  describe("resolveAlias() method", () => {
    ...
    it("resolves known aliases or return null when alias is unknown", done => {
      biblioDB
        .addAll(data)
        .then(() => biblioDB.resolveAlias("whatwg-dom"))
        .then(result => expect(result).toEqual("WHATWG-DOM"))
        .then(() => biblioDB.resolveAlias("does not exist"))
